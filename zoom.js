...
function savesettings() {
    ...
    if (newaccenttheme) {
        chrome.storage.local.set({ "zoom-accenttheme": newaccenttheme === "*" ? `#373737` : newaccenttheme })
    }
    if (newcolortheme) {
        chrome.storage.local.set({ "zoom-colortheme": newcolortheme === "*" ? `#fff` : newcolortheme })
    }
    if (newbuttontext) {
        chrome.storage.local.set({ "zoom-buttontext": newbuttontext === "*" ? "" : newbuttontext })
    }
    if (newpostimage) {
        chrome.storage.local.set({ "zoom-postimage": newpostimage === "*" ? "" : newpostimage })
    }
    if (newsecondarytheme) {
        chrome.storage.local.set({ "zoom-secondarytheme": newsecondarytheme === "*" ? `#373737` : newsecondarytheme })
    }
    if (newbackgroundcolor) {
        chrome.storage.local.set({ "zoom-backgroundcolor": newbackgroundcolor === "*" ? `#1b1b1b` : newbackgroundcolor })
    }
    if (extracss) {
        chrome.storage.local.set({ "zoom-extracss": extracss === "*" ? "" : extracss })
    }
...
togglebutton.onclick = function () {
    if (darkmodestate) {
        ...
        darkmodestate = false
        chrome.storage.local.set({ "zoom-darkmode": false })
    } else {
        insertstyle(accenttheme, postimage, backgroundcolor, extracss, colortheme, secondarytheme)
        document.querySelector("#dark-mode-switch").innerHTML = "🌞"
        darkmodestate = true
        chrome.storage.local.set({ "zoom-darkmode": true })
    }
}
...
chrome.storage.local.get(["zoom-darkmode", "zoom-accenttheme", "zoom-buttontext", "zoom-postimage", "zoom-backgroundcolor", "zoom-extracss", "zoom-colortheme", "zoom-secondarytheme"], function (result) {
