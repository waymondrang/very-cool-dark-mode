...
// DEBUGGING
if (debugmode) {
    chrome.storage.local.set({ greetingemoji: "" })
}
...
function savesettings() {
    ...
    if (newbordertheme) {
        chrome.storage.local.set({ bordertheme: newbordertheme === "*" ? `#373737` : newbordertheme })
    }
    if (newshadetheme) {
        chrome.storage.local.set({ shadetheme: newshadetheme === "*" ? `#212121` : newshadetheme })
    }
    if (newcolortheme) {
        chrome.storage.local.set({ colortheme: newcolortheme === "*" ? `#fff` : newcolortheme })
    }
    if (newgreetingemoji) {
        chrome.storage.local.set({ greetingemoji: newgreetingemoji === "*" ? "" : newgreetingemoji })
    }
    if (newstudentimage) {
        chrome.storage.local.set({ studentimage: newstudentimage === "*" ? "" : newstudentimage })
    }
    if (extracss) {
        chrome.storage.local.set({ extracss: extracss === "*" ? "" : extracss })
    }
...
modalbutton.onclick = function () {
    if (togglemodal) {
        ...
        chrome.storage.local.get(["extracss"], function (result) {
...
chrome.storage.local.get(["bordertheme", "shadetheme", "colortheme", "darkmode", "greetingemoji", "studentimage", "extracss"], function (result) {
